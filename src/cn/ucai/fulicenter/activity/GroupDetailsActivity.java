...
//        adapter.add(members);
    ...
    private void addMembersToGroup(final String[] newmembers) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    boolean result = NetUtil.addGroupMembers(group.getName(), newmembers);
                    if (result) {
                        ArrayList<UserBean> contactList = SuperWeChatApplication.getInstance().getContactList();
                        for (int i = 0; i < newmembers.length; i++) {
                            UserBean u = new UserBean(newmembers[i]);
                            int id = contactList.indexOf(u);
                            if (id >= 0) {
                                mGroupMembers.add(u);
                            }
                        }
    ...
    private class GridAdapter extends BaseAdapter {
        ...
        public GridAdapter(Context context, int textViewResourceId, ArrayList<UserBean> objects) {
            this.list = objects;
        ...
        public void add(NetworkImageView avatar) {
...
//				UserUtils.setUserAvatar(mContext, username, holder.avatar);
                UserUtils.setUserBeanNick(user.getUserName(), holder.textView);
                UserUtils.setUserBeanAvatar(user.getUserName(), holder.avatar);
